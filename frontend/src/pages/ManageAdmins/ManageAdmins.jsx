...
const ManageAdmins = () => {
  ...
  const handleAssignRole = async (userId, roleId) => {
    try {
      const res = await axios.put(`http://localhost:5000/api/users/${userId}/role`, { roleId }, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setAdmins(admins.map(admin => admin.id === userId ? { ...admin, role: res.data.role_name } : admin));
                ...
                <td>
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (selectedRole) handleAssignRole(admin.id, selectedRole.id);
                    }}
                  >
                    {roles.map(role => (
