...
export const AuthProvider = ({ children }) => {
  ...
  const signup = async (name, email, password, roleCode, phone, address) => {
    try {
      await axios.post('http://localhost:5000/api/signup', { name, email, password, roleCode, phone, address });
      return { success: true };
    } catch (err) {
  ...
  const adminLogin = async (email, password) => {
    try {
      ...
      localStorage.setItem('token', res.data.token);
      setUser(res.data.user);
      setRole(res.data.user.role); // Use actual role from response
      return { success: true };
    } catch (err) {
